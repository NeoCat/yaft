...
bool cmap_init(int fd, struct fb_info_t *info, cmap_t *cmap, int colors)
{
	...
	for (int i = 0; i < colors; i++) {
		r = (i >> info->red.offset)   & bit_mask[info->red.length];
		g = (i >> info->green.offset) & bit_mask[info->green.length];
		b = (i >> info->blue.offset)  & bit_mask[info->blue.length];

		/* XXX: maybe only upper order byte is used */
		*(cmap->red   + i) = r * bit_mask[16] / bit_mask[info->red.length];
		*(cmap->green + i) = g * bit_mask[16] / bit_mask[info->green.length];
		*(cmap->blue  + i) = b * bit_mask[16] / bit_mask[info->blue.length];
	}
...
static inline uint32_t color2pixel(struct fb_info_t *info, uint32_t color)
{
	...
	r = r >> (BITS_PER_BYTE - info->red.length);
	g = g >> (BITS_PER_BYTE - info->green.length);
	b = b >> (BITS_PER_BYTE - info->blue.length);
...
bool init_indexcolor(int fd, struct fb_info_t *info, cmap_t **cmap, cmap_t **cmap_orig)
{
	...
	*cmap_orig = cmap_create(colors);
	...
	if (!cmap_init(fd, info, *cmap, colors))
		goto cmap_init_err;
